...
const maps = {
    ...
    "Nell's Diner": {
        image: "/maps/diner.png",
        cursedObjects: [
            { type: "Tarot Cards", location: "Behind the counter, next to the till." },
            { type: "Ouija Board", location: "In breaker room on boxes." },
            { type: "Music Box", location: "On table next to coffee machine in Manager's office." },
            { type: "Voodoo Doll", location: "One of the table and chairs in the Diner" },
            { type: "Haunted Mirror", location: "On chair next to vending machine in employee's room." },
            { type: "Summoning Circle", location: "In men's toilets." },
            { type: "Monkey Paw", location: "On chopping board in the kitchen." }
        ]
    }
};
...
function initializeGhostDatabase() {
    ...
    container.innerHTML = Object.keys(ghosts).map(ghostName => {
        ...
        const escapedName = ghostName.replace(/'/g, "\\'");
        return `<div class="ghost-card" onclick="showGhostDetails('${escapedName}')">
            <div class="ghost-icon">
                <img src="${ghost.icon}" alt="${ghostName}" onerror="this.style.display='none'; this.parentNode.innerHTML='👻';">
            </div>
            <div class="ghost-name">${ghostName}</div>
            <div class="evidence-list">
                ${ghost.evidence.map(ev => `<span class="evidence-badge">${getEvidenceName(ev)}</span>`).join('')}
            </div>
        </div>`;
    }).join('');
}
...
function initializeMapDatabase() {
    ...
    container.innerHTML = Object.keys(maps).map(mapName => {
        ...
        return `<div class="map-card" onclick="showMapDetails('${escapedName}')">
            <div class="map-name">${mapName}</div>
            <div class="map-image">
                <img src="${map.image}" alt="${mapName}" onerror="this.style.display='none'; this.parentNode.innerHTML='🗺️';">
            </div>
            <div class="cursed-objects-preview">
                <div class="object-count">${map.cursedObjects.length} Cursed Objects</div>
                <div style="margin-top: 10px; color: #bbb;">
                    ${map.cursedObjects.map(obj => obj.type).join(', ')}
                </div>
            </div>
        </div>`;
    }).join('');
...
function updateResults() {
    ...
    resultContainer.innerHTML = `<div class="possible-ghosts-grid">
        ${matchingGhosts.map(ghostName => {
            ...
            return `<div class="ghost-card" onclick="showGhostDetails('${escapedName}')">
                ...
                <div class="ghost-name">${ghostName}</div>
                <div class="evidence-list">
                    ${ghost.evidence.map(ev => `<span class="evidence-badge ${selectedEvidence.includes(ev) ? 'selected' : ''}">${getEvidenceName(ev)}</span>`).join('')}
                </div>
            </div>`;
        }).join('')}
    </div>`;
